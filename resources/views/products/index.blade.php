...
@section('title', $currentCategory ? $currentCategory->name : 'Tous les produits')
...
<div style="background:linear-gradient(135deg,var(--bg-base) 0%,var(--hero-gradient-end) 100%);border-bottom:1px solid var(--border);padding:36px 0 0;">
    <div class="container">
        ...
        {{-- Filtres catégories (mobile/tablette — horizontal scroll) --}}
        <div class="d-lg-none prod-mobile-tabs">
            <a href="{{ route('products.index') }}"
               class="cat-tab-pill {{ !$currentCategory ? 'active' : '' }}">
                Tous
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('products.index', ['category' => $cat->id]) }}"
               class="cat-tab-pill {{ $currentCategory?->id === $cat->id ? 'active' : '' }}">
                {{ $cat->name }}
            </a>
            @endforeach
        </div>
            ...
            <div style="position:sticky;top:82px;">
                ...
                <div class="card border-0 shadow-sm" style="overflow:hidden;">
                    <div style="padding:13px 18px;border-bottom:1px solid var(--border);">
                        <span style="font-size:.71rem;text-transform:uppercase;letter-spacing:1.8px;color:var(--text-muted);font-weight:700;">Filtrer par catégorie</span>
                    </div>
                    <ul style="list-style:none;padding:6px 0;margin:0;">
                        <li>
                            <a href="{{ route('products.index') }}"
                               class="cat-sidebar-link {{ !$currentCategory ? 'is-active' : '' }}">
                        ...
                        <li>
                            <a href="{{ route('products.index', ['category' => $cat->id]) }}"
                               class="cat-sidebar-link {{ $currentCategory?->id === $cat->id ? 'is-active' : '' }}">
                                <span>{{ $cat->name }}</span>
        ...
        <div class="col-lg-9">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <h5 style="color:var(--text-primary);font-weight:700;margin-bottom:8px;">Aucun produit disponible</h5>
                    <p style="color:var(--text-muted);font-size:.9rem;max-width:300px;margin:0 auto 20px;line-height:1.6;">
                        Cette catégorie ne contient pas encore de produits ou est temporairement indisponible.
                    </p>
